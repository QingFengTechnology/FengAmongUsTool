...
class ServerLoader(QObject):
    ...
    def loadServers(self):
        ...
        # 等待任一线程完成，设置超时时间
        self.load_event.wait(timeout=30)  # 30秒超时
    ...
    def run(self):
        ...
        try:
            ...
            for server in self.servers:
                ...
                # 等待任一线程完成，设置超时时间
                server_load_event.wait(timeout=30)  # 30秒超时
...
def fetchAndInstallServers(servers, logCallback):
    ...
    try:
        ...
        # 启动配置加载
        config_loader.start()
        config_event.wait(timeout=60)  # 等待加载完成，设置60秒超时
        ...
        # 如果所有配置都重复，则提示安装取消
        if duplicate_count > 0 and not unique_server_regions:
            logCallback("所有服务器配置都是重复项，跳过安装")
            # 恢复只读属性（跨平台处理）
            if platform.system() == "Windows":
                region_file_path.chmod(stat.S_IREAD)
            else:
                region_file_path.chmod(0o444)
...
def getEnabledServers(serverConfig):
    ...
    # 过滤启用的服务器配置
    enabled_configs = []
    for config in serverConfig.values():
        if config['enabled']:
...
def toggleServerState(serverConfig, serverId, state):
    """切换服务器状态"""
    serverConfig[serverId]['enabled'] = (state == 2)
    return serverConfig